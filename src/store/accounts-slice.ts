...
export const modifyPrefs = createAsyncThunk<any, ModifyPrefsArgs>(
	...
	async (mods, { getState }) => {
		...
		if (mods.permissions) {
			requests.ModifyPermissionRequest = `<BatchRequest xmlns="urn:zimbra" onerror="stop"><RevokeRightsRequest xmlns="urn:zimbraAccount" requestId="0">${
			...
			</GrantRightsRequest></BatchRequest>`;
		}
		const res = await fetch('/service/soap/BatchRequest', {
			...
			<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
				...
				<soap:Body>
					...
					${requests.ModifyPermissionRequest ?? ''}
				</soap:Body>
			</soap:Envelope>
		`
		});
